...
- [ ] Verify camera angle and lighting
- [ ] Print test license plates (if needed)
- [ ] Charge laptop / check power supply
- [ ] Test on demo network (if different)
- [ ] Prepare backup USB camera
- [ ] Export sample database queries
- [ ] Take screenshots of working system

### Demo Day Morning

- [ ] Arrive early (1 hour before)
- [ ] Setup hardware in demo location
- [ ] Connect camera and verify feed
- [ ] Start system and check logs
- [ ] Run test vehicle pass
- [ ] Verify database recording
- [ ] Have backup plan ready (CPU mode, USB camera)
- [ ] Clear old test data: `rm anpr.db` then restart

### During Demo

**Demo Script:**
1. Show system startup logs
2. Point camera at parking lot / road
3. Explain detection → tracking → OCR → event flow
4. Show live detections on screen
5. Drive vehicle past camera
6. Query database to show recorded event:
   ```sql
   sqlite3 anpr.db "SELECT * FROM events ORDER BY timestamp DESC LIMIT 1;"
   ```
7. Show system stats in logs

**Key Talking Points:**
- Real-time processing (show FPS)
- Automatic vehicle detection (any class)
- License plate reading with temporal fusion
- Entry/Exit event tracking
- Database persistence
- Fault tolerance (camera reconnect)

### If Something Goes Wrong

**Backup Plan A:** Fall back to CPU mode
```yaml
detection:
  device: cpu
```

**Backup Plan B:** Use USB webcam instead of IP camera
```yaml
cameras:
  - source: "0"
```

**Backup Plan C:** Show pre-recorded data
```bash
# Export events beforehand
sqlite3 anpr.db ".output events.txt" "SELECT * FROM events;"
cat events.txt
```
